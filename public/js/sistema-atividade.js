...
class SistemaAtividades {
    ...
    vincularEventos() {
        document.addEventListener('click', (e) => {
            if (e.target.matches('[data-action="create-activity"]')) {
                window.abrirSecaoFlex('create-activity-modal')
            }
    ...
    async salvarAtividade() {
        ...
        if(!tipo) {
            showToast('Por favor, selecione o tipo da atividade', 'error');
            return;
        }
    ...
    async atualizarStatusRespostas(idAtividade) {
        try {
            ...
            respostas.respostas.forEach(resposta => {
                EstadoSala.respostasAtividade[resposta.studentId] = {
                    ...resposta,
...
window.responderComoAluno = responderComoAluno
