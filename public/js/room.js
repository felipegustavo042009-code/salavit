...
const GerenciadorSala = {
    ...
    criarSala: async () => {
        try {
            EstadoSala.idUsuario = localStorage.getItem('idUsuario');
            console.log('ID do usuário ao criar a sala:', EstadoSala.idUsuario);
            ...
            localStorage.setItem('codigoSala', resultado.codigo);
    ...
    encerrarSala: async () => {
        ...
        if (idSala) {
            try {
                ...
                showToast('Encerrando sala isso pode demorar um pouco...', 'info');
    ...
    entrarSala: async (codigoSala) => {
        try {
            ...
            EstadoSala.idUsuario = localStorage.getItem('idUsuario');
            console.log('ID do usuário ao entrar na sala:', EstadoSala.idUsuario);
    ...
    configurarEscutadoresSocket: () => {
        console.log('Configurando escutadores de sala para o aluno');
        const idUsuario = localStorage.getItem('idUsuario');
        const idSala = localStorage.getItem('idSala');

        api.onQuizAtualizado((data) => {
            if (data.salaId === idSala) {
                showToast('Novo quiz disponível', 'info');
        ...
        api.onMateriaisAtualizados((data) => {
            if (data.salaId === idSala) {
                showToast('Novo material disponível', 'info');
                window.sistemaMateriais.carregarMateriais();
            }
        });
    ...
    escutarAtualizacoesSala: () => {
                    ...
                    .then(alunos => {
                        ...
                        if (EstadoSala.atividadeAtivaId) {
                            if (window.sistemaAtividades) {
                                window.sistemaAtividades.atualizarStatusRespostas(EstadoSala.atividadeAtivaId);
                            }
                        }
                    });
            }
        ...
        api.onMaoLevantada((data) => {
            if (data.salaId === idSala) {
                showToast('Um aluno levantou a mão', 'info');
    ...
    listarAlunosConectados: () => {
        ...
        if (estaListandoRespostas) {
            ...
            EstadoSala.alunosConectados.forEach(aluno => {
                const respondeu = EstadoSala.respostasAtividade &&
                    EstadoSala.respostasAtividade[aluno.id];
        ...
        listaAlunos.innerHTML = alunosOrdenados.map(aluno => {
            ...
            const resposta = EstadoSala.respostasAtividade[aluno.id];
...
const GerenciadorAluno = {
    ...
    atualizarUIAluno: () => {
        ...
        console.log('Atualizando UI do aluno para a sala:', codigoSala);
        const idSala = localStorage.getItem('idSala');
